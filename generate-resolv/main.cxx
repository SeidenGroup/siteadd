...
static auto QtocRtvTCPA = ILEFunction<void*, int*, const char*, ERRC0100*>("QSYS/QTOCNETSTS");
...
{
	printf("options timeout:%d attempts:%d", tcp1400->Time_Interval, tcp1400->Retries);
...
print_nameservers(Qtoc_RtvTCPA_TCPA1100_t *tcp1100, Qtoc_RtvTCPA_TCPA1400_t *tcp1400)
{
	...
	for (int i = 0; i < tcp1400->Number_Of_DNS_Addresses; i++) {
...
{
	int outlen = 1000000;
	Qtoc_RtvTCPA_TCPA1100_t *out = malloc(outlen);
	memset(out, 0, outlen);
	out->Bytes_Available = outlen;
	ERRC0100 err = { 0 };
	err.bytes_in = sizeof (err);
	
	QtocRtvTCPA((void*)out, &outlen, TCPA1400_name.value, &err);
	...
	print_domain((Qtoc_RtvTCPA_TCPA1400_t*)out->data);
	print_search((Qtoc_RtvTCPA_TCPA1400_t*)out->data);
	print_options((Qtoc_RtvTCPA_TCPA1400_t*)out->data);
	print_nameservers(out, (Qtoc_RtvTCPA_TCPA1400_t*)out->data);
}
