...
; (SG) This includes additional changes over the stock PHP production INI
; defaults. They have been marked (SG) when possible.
...
; (SG) We turn this on by default (unlike upstream PHP) for compatibility
; with legacy code.
short_open_tag = On
...
;error_log = syslog
; (SG) We recommend logging this to a file, for timestamps (which doesn't happen on stderr).
...
session.save_path = "xWWWDIR/sessions"
